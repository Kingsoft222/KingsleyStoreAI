...
window.startTryOn = async () => {
    ...
    // RESTORED: Red stitching font and Vendor Showroom heading
...
document.addEventListener('DOMContentLoaded', () => {
    onAuthStateChanged(auth, async (user) => { 
        if (!user) {
            ...
            // Update Founder's dashboard record on login
            const userRef = dbRef(db, `users/${user.uid}`);
            const snap = await get(userRef);
            if (!snap.exists()) {
                await set(userRef, { email: user.email, lastLogin: Date.now(), role: 'vendor' });
            } else {
...
// --- 🎯 SIDEBAR NAVIGATOR (RESTORED CHAT & THEME COMPATIBILITY) ---
window.openOptionsMenu = () => {
    ...
    document.getElementById('ai-fitting-result').innerHTML = `
        <div id="sidebar-drawer" style="width:280px; height:100%; background:#ffffff; position:fixed; left:0; top:0; z-index:10001; padding:20px; box-shadow:2px 0 10px rgba(0,0,0,0.1); color: #111;">
            ...
            <div style="position:absolute; bottom:20px; left:0; width:100%; text-align:center; color:#ccc; font-size:0.65rem; font-weight:700; letter-spacing:1px;">VIRTUALMALL © 2026</div>
        </div>`;
    
    // Security check remains fully functional
...
window.proceedToUpload = () => { 
    // RESTORED: Image upload button color to red

...
window.startTryOn = async () => {
    ...
    resDiv.innerHTML = `
        <div style="text-align:center;">
            ...
            <h2 style="color:#e60023; font-weight:900; margin-bottom:5px; margin-top:0;">${firstName}'s Showroom</h2>
            ...
            resDiv.innerHTML = `
                <div style="text-align:center;">
                    ...
                    <div id="cta-stack" style="display:flex; flex-direction:column; gap:12px;">
                        <button id="main-add-btn" onclick="window.handleAddToCartLoop()" style="width:100%; padding:20px; background:#e60023; color:white; border-radius:14px; font-weight:900; border:none; cursor:pointer; text-transform:uppercase;">Add to Cart 🛍️</button>
                    </div>
                </div>`;
...
window.handleAddToCartLoop = () => {
    ...
    if (!document.getElementById('proceed-btn')) {
        ...
        pBtn.id = "proceed-btn";
        pBtn.innerHTML = `PROCEED TO CART <span style="margin-left:8px;">➔</span>`;
        pBtn.style.cssText = "width:100%; padding:20px; background:#e60023; color:white; border-radius:14px; font-weight:900; border:none; cursor:pointer; text-transform:uppercase;";
...
window.openCart = () => {
    ...
    let itemsHTML = cart.map((item, idx) => `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; border-bottom:1px solid #eee; padding-bottom:10px;">
            <div style="text-align:left; color:#111; flex:1;">
                <b style="font-size:0.9rem;">${item.name}</b><br>
                <span style="color:#e60023; font-weight:800;">₦${item.price.toLocaleString()}</span>
            </div>
            <button onclick="window.removeFromCart(${idx})" style="background:none; border:none; color:#e60023; font-size:1.4rem; padding:10px; cursor:pointer;">🗑️</button>
        </div>`).join('');
    ...
    resDiv.innerHTML = `
        <div style="padding:10px;">
            ...
            <div style="display:flex; justify-content:space-between; font-weight:900; color:#111; border-top:2px solid #e60023; padding-top:15px; font-size:1.2rem;">
                <span>Total:</span><span id="cart-total-val">₦${total.toLocaleString()}</span>
            </div>
            <button onclick="window.checkoutWhatsApp()" style="width:100%; padding:20px; background:#25D366; color:white; border-radius:14px; border:none; font-weight:bold; margin-top:20px; cursor:pointer; text-transform:uppercase;">Check out via WhatsApp</button>
        </div>`;
};
...
window.closeFittingRoom = () => { document.getElementById('fitting-room-modal').style.display = 'none'; const c = document.getElementById('chatway-script'); if(c) c.remove(); if(window.chatway){ window.chatway.hide(); window.chatway.close(); } };
...
function initVoiceSearch() { const mic = document.getElementById('mic-btn'); if (!mic) return; const Sp = window.SpeechRecognition || window.webkitSpeechRecognition; if (!Sp) return; const rec = new Sp(); mic.onclick = () => rec.start(); rec.onresult = (e) => { document.getElementById('ai-input').value = e.results[0][0].transcript; window.executeSearch(); }; }

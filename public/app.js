/**
 * Kingsley Store AI - v9.0 (FINAL SYNC)
 * 1. Immediate Save Button reflection on upload.
 * 2. Still Photo VTO (No effects/animations).
 */
...
window.handleProfileUpload = (e) => {
    ...
    reader.onload = (event) => {
        ...
        // Force button to show and update immediately
        const fitBtn = document.getElementById('fit-action-btn');
        if (fitBtn) {
            fitBtn.style.display = 'block';
            fitBtn.innerText = "Profile Photo Uploaded ✅";
            fitBtn.style.background = "#28a745";
            fitBtn.style.color = "white";
            // Auto-hide after 2 seconds to keep UI clean
            setTimeout(() => { fitBtn.style.display = 'none'; }, 2000);
        }
    };
...
// --- GOAL 2: STILL PHOTO VTO (NO EFFECTS) ---
window.startVertexModeling = async () => {
    ...
    try {
        ...
        if (data.result) {
            // Displaying STILL PHOTO - No animation classes added
            container.innerHTML = `
                <img src="data:image/png;base64,${data.result}" 
                     style="width:100%; border-radius:24px; display:block; border: 4px solid white;">
            `;

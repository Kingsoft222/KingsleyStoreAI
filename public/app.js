        ...
        if (data) {
            ...
            if (container) {
                container.innerHTML = `
                    ...
                    <div class="split-card" onclick="window.quickSearch('${data.label2}')"><h4>${data.label2 || 'Bespoke'}</h4><p>Shop now</p></div>`;
            }
...
window.openCart = () => {
    ...
    resDiv.innerHTML = `
        <div style="padding:10px;">
            <div class="close-preview-x" onclick="window.closeFittingRoom()">×</div>
            <h2 style="color:#e60023; font-weight:900; margin-bottom:20px;">CART SUMMARY</h2>
            ${cart.map((item, idx) => `<div style="display:flex; justify-content:space-between; margin-bottom:15px; border-bottom:1px solid #eee; padding-bottom:10px; color:#000;"><div><b>${item.name}</b><br>₦${item.price.toLocaleString()}</div><button onclick="window.removeFromCart(${idx})" style="background:none; border:none; color:red; font-size:1.2rem;">✕</button></div>`).join('')}
            <div style="display:flex; justify-content:space-between; font-weight:900; font-size:1.2rem; margin-top:20px; border-top:2px solid #e60023; padding-top:15px; color:#000;"><span>Total:</span><span>₦${total.toLocaleString()}</span></div>
...
window.promptShowroomChoice = (id) => {
    ...
    document.getElementById('ai-fitting-result').innerHTML = `
        <div style="text-align:center;">
            ...
            <h3 style="color:#000; margin-top:15px;">${selectedCloth.name}</h3>
...
window.proceedToUpload = () => {
    document.getElementById('ai-fitting-result').innerHTML = `
        <div style="text-align:center; padding:20px;">
            ...
            <h2 style="color:#e60023; font-weight:900;">FINISH LOOK</h2>
            <p style="color:#666; margin-bottom:25px;">Upload your photo showing from head to toe for better results</p>
...
// ... Inspection, Search, Close, and Other logics remain untouched ...
...
window.executeSearch = () => { /* logic untouched */ };
...
function initVoiceSearch() { /* logic untouched */ }
function applyDynamicThemeStyles() { const isDarkMode = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches; const styleTag = document.createElement('style'); styleTag.innerHTML = `#store-name-display, #dynamic-greeting { color: ${isDarkMode ? 'white' : 'black'} !important; }`; document.head.appendChild(styleTag); }
...
window.checkoutWhatsApp = async () => { const total = cart.reduce((s, i) => s + i.price, 0); const msg = `Order Total: ₦${total.toLocaleString()}\nItems: ${cart.map(i => i.name).join(', ')}`; window.location.assign(`https://wa.me/${storePhone}?text=${encodeURIComponent(msg)}`); };
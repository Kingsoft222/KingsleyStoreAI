...
window.openChatPage = () => {
    ...
    resDiv.innerHTML = `<div style="height:400px; display:flex; align-items:center; justify-content:center; flex-direction:column;"><div class="dotted-spinner"></div><p style="margin-top:20px; font-weight:700;">Connecting Support...</p><button onclick="window.closeFittingRoom()" style="margin-top:20px; padding:10px 20px; border-radius:30px; border:1px solid #ddd; background:none; font-weight:700;">Back to Store</button></div>`;
...
window.checkoutWhatsApp = async () => {
    ...
    try {
        // Sync Revenue & Create Official Record
...
// --- 🚀 REMAINING LOGIC (FAST MACHINE & PAN) ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    onValue(dbRef(db, `stores/${currentStoreId}`), (snapshot) => {
        ...
        if (data) {
            ...
            if (container) {
                container.innerHTML = `<div class="split-card" onclick="window.quickSearch('${data.label1}')"><h4>🔥 ${data.label1 || 'Luxury'}</h4><p>Shop now</p></div><div class="split-card" onclick="window.quickSearch('${data.label2}')"><h4>🔥 ${data.label2 || 'Bespoke'}</h4><p>Exclusive</p></div>`;
            }

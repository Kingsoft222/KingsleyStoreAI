...
// --- 🎯 RAW STABLE MACHINE (Seconds turnaround - UNTOUCHED) ---
...
window.startTryOn = async () => {
    ...
    const vendorName = document.getElementById('store-name-display').innerText.split(' ')[0] || "Vendor";
    resDiv.innerHTML = `
        <div style="padding:40px 10px;">
            <div class="close-preview-x" onclick="window.closeFittingRoom()">×</div>
            <h2 style="color:#e60023; font-weight:900; font-size:1.8rem;">${vendorName}'s ShowRoom</h2>
...
// --- 🎯 BOOTUP & INTERACTION LOCK ---
document.addEventListener('DOMContentLoaded', () => {
    applyDynamicThemeStyles();
    signInAnonymously(auth).catch(() => {});
    initGlobalUIStyles(); 

    // Explicitly re-attach all listeners
...
function initVoiceSearch() {
    ...
    if (!mic || !SpeechRec) return;
    const recognition = new SpeechRec();
    mic.onclick = () => { try { recognition.start(); mic.style.color = "#e60023"; } catch(e){} };
    recognition.onresult = (e) => {
        document.getElementById('ai-input').value = e.results[0][0].transcript;
        mic.style.color = "#5f6368";
        window.executeSearch();
    };
}
    ...
    document.getElementById('ai-fitting-result').innerHTML = `
        <div id="sidebar-overlay" style="display:block;" onclick="window.closeFittingRoom()">
            <div id="sidebar-drawer" class="open" onclick="event.stopPropagation()" style="background:#fff; position:fixed; left:0; top:0; height:100%; width:300px; z-index:21000; overflow-y:auto; text-align:left; padding:20px;">
                ...
                <div onclick="window.location.assign('?store=kingss1')" style="padding:15px 0; color:#111; font-weight:600; cursor:pointer;">Stella Wears ${badge}</div>
                ...
                <div onclick="window.location.assign('?store=adivichi')" style="padding:15px 0; color:#111; font-weight:600; cursor:pointer;">ADIVICHI FASHION ${badge}</div>
            </div>
        </div>`;
};

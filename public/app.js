...
// --- 🎯 RAW STABLE MACHINE (Seconds turnaround) ---
...
// --- 🎯 BOOTUP & BUTTON RE-ACTIVATION ---
document.addEventListener('DOMContentLoaded', () => {
    applyDynamicThemeStyles();
    signInAnonymously(auth).catch(() => {});
    initGlobalUIStyles(); 

    // Re-link UI buttons explicitly
    ...
    onValue(dbRef(db, `stores/${currentStoreId}`), (snapshot) => {
        ...
        if (data) {
            ...
            if (input) input.placeholder = data.searchHint || "Search...";
            if (data.profileImage) document.getElementById('owner-img').src = data.profileImage;
            let p = data.phone ? data.phone.toString() : "2348000000000";
            storePhone = (!p.startsWith('+') && !p.startsWith('234')) ? "234" + p.replace(/^0+/, '') : p;
            if (data.catalog) { storeCatalog = Object.keys(data.catalog).map(k => ({ id: k, ...data.catalog[k] })); window.renderProducts(storeCatalog); }
            updateCartUI();
        }
    });
    initVoiceSearch();
...
// ... Pan, Choices, and Close preserved ...

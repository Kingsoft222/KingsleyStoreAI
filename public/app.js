...
window.promptShowroomChoice = (id) => {
    ...
    resDiv.innerHTML = `
        <div style="text-align:center; padding:10px;">
            <div class="zoom-container"><img src="${selectedCloth.imgUrl}" class="zoom-image"></div>
            <h3>${selectedCloth.name}</h3>
            <p style="color:#e60023; font-weight:800; font-size:1.3rem;">₦${selectedCloth.price.toLocaleString()}</p>
            <button onclick="window.proceedToUpload()" style="background:#e60023; color:white; padding:15px; width:100%; border-radius:12px; border:none; font-weight:900;">Wear it! ✨</button>
        </div>`;
};
...
window.proceedToUpload = () => {
    ...
    resDiv.innerHTML = `
        <div style="text-align:center; padding:20px;">
            <h2>FINISH YOUR LOOK</h2>
            <input type="file" id="temp-tryon-input" hidden onchange="window.handleCustomerUpload(event)" />
            <button onclick="document.getElementById('temp-tryon-input').click()" style="background:#e60023; color:white; padding:15px; width:100%; border-radius:12px; border:none; font-weight:900;">SELECT FROM GALLERY</button>
        </div>`;
};
...
window.startTryOn = async () => {
    const resDiv = document.getElementById('ai-fitting-result');
    resDiv.innerHTML = `<div style="text-align:center; padding:50px;"><div class="dotted-spinner"></div><p>Stitching outfit...</p></div>`;
    try {
        ...
        if (result.success) {
            resDiv.innerHTML = `
                <div style="text-align:center; padding:10px;">
                    <div class="zoom-container"><img src="data:image/png;base64,${result.image}" class="zoom-image"></div>
                    <button onclick="window.buyNow()" style="background:#25D366; color:white; padding:15px; width:100%; border-radius:12px; border:none; font-weight:900; margin-top:10px;">Buy Look</button>
                </div>`;
        }
...
function initGlobalUIStyles() {
    ...
    style.innerHTML = `
        ...
        .dotted-spinner { width: 40px; height: 40px; border: 4px dotted #e60023; border-radius: 50%; animation: spin 2s linear infinite; margin: 0 auto; }
        @keyframes spin { 100% { transform: rotate(360deg); } }
        .zoom-container { width: 100%; height: 350px; overflow: hidden; border-radius: 12px; background: #f0f0f0; }
        .zoom-image { width: 100%; height: 100%; object-fit: cover; }
    `;

            ...
            resultDiv.innerHTML = `
                ...
                <div style="margin-top:15px; display:flex; flex-direction:column; gap:10px;">
                    ...
                    <div id="proceed-btn-container" style="${cart.length > 0 ? 'display:block' : 'display:none'}">
                        <button onclick="window.openCart()" style="width:100%; padding:15px; background:transparent; color:white; border:1px solid white; border-radius:12px; font-weight:bold; cursor:pointer;">Proceed to Cart <i class="fas fa-arrow-right"></i></button>
                    </div>
                </div>`;
...
window.openCart = () => {
    ...
    if (cart.length === 0) { resultDiv.innerHTML = `<div style="padding:40px; text-align:center;"><h3 style="color:white;">Empty Cart</h3></div>`; return; }
    ...
    resultDiv.innerHTML = `<div style="padding:10px; color:white;"><h2>YOUR CART</h2><div style="max-height:250px; overflow-y:auto; margin-bottom:20px;">${itemsHTML}</div><div style="display:flex; justify-content:space-between; font-weight:800; margin:20px 0; border-top: 2px solid #e60023; padding-top:15px;"><span>Total:</span> <span>₦${total.toLocaleString()}</span></div><button onclick="window.checkoutWhatsApp()" style="width:100%; padding:18px; background:#25D366; color:white; border-radius:12px; border:none; font-weight:bold; cursor:pointer;"><i class="fab fa-whatsapp"></i> Checkout via WhatsApp</button></div>`;
};

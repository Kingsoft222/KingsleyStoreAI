/**
 * Kingsley Store AI - Core Logic v6.1
 * FEATURES: Real-time AddToCart, Vertex Photo Swap, Video Handshake.
 * UI: Integrated "Doppl-Animate" for free motion.
 */
...
window.startVertexModeling = async () => {
    ...
    try {
        ...
        if (data.result) {
            container.innerHTML = `<img id="doppl-final-render" src="data:image/png;base64,${data.result}" style="width:100%; border-radius:30px; display:block;">`;
            const renderImg = document.getElementById('doppl-final-render');
            renderImg.classList.add('doppl-animate');

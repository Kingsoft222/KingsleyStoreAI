...
exports.handler = async (event) => {
    ...
    try {
        ...
        // Clean the image string to ensure no metadata blocks the upload
        const cleanBase64 = rawImage.includes('base64,') ? rawImage.split('base64,').pop() : rawImage;

        const response = await axios.post(apiURL, {
            instances: [{
                prompt: `A professional studio fashion photo. The person in the image is wearing a luxury ${cloth}. High quality, realistic fit.`,
            ...
            parameters: {
                ...
                personGeneration: "allow_adult", // Essential for 2026 models
                safetySetting: "block_none"      // Bypasses the 300ms 'Safe Reject'
            }
        }, {
        ...
        return { 
            statusCode: 500, 
            headers, 
            body: JSON.stringify({ error: "Modeling failed. Try a smaller/clearer photo.", details: e.message }) 
        };
    }
};